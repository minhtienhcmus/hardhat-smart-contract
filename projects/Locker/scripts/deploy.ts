...
const main = async () => {
  ...
  console.log("Deploying to network:", network);
    ...
    console.log("Deploying CbaseRewardLockerV2");
  ...
  console.log("======================")
  const  factory = await ethers.getContractFactory("CbaseRewardLockerV2");
  const ffac = (await factory.deploy(config.ADMIN[name]));
  await ffac.deployed();
  console.log("CbaseRewardLockerV2:",ffac.address);

  await verify("CbaseRewardLockerV2", ffac.address,[config.ADMIN[name]]);
