...
contract LockFactory is ILockFactory, Ownable2Step, IERC721Receiver, ReentrancyGuard {
    ...
    */
    function lock (LockParams calldata params) external payable override nonReentrant returns (uint256) {
        ...
        (position,info) = params.nftPositionManager.positions(params.nft_id);
        IFactory factory = IFactory(params.nftPositionManager.factory());
        address pool = factory.getPool(info.token0, info.token1, info.fee);
        int24 maxTick = tickSpacingToMaxTick(factory.feeAmountTickSpacing(info.fee));
        ...
        if (position.tickLower != -maxTick && position.tickUpper != maxTick) {
            ...
            params.nftPositionManager.collect(IBasePositionManager.CollectParams(nftId, params.dustRecipient, type(uint128).max, type(uint128).max));
        }
    ...
    function emitLockEvent (uint256 _lockId) private {
        ...
        IBasePositionManager.Position memory position;
        IBasePositionManager.PoolInfo memory info;
        (position,info) = newLock.nftPositionManager.positions(newLock.nft_id);
        emit onLock(
            newLock.lock_id, 
    ...
    function _convertPositionToFullRange (IBasePositionManager _nftPositionManager, uint256 _tokenId, IBasePositionManager.Position memory _position, int24 _maxTick, address _dustRecipient) private returns (uint256) {
        _nftPositionManager.removeLiquidity(IBasePositionManager.RemoveLiquidityParams(_tokenId, _position.liquidity, 0, 0, block.timestamp));
        _nftPositionManager.burnRTokens(IBasePositionManager.BurnRTokenParams(_tokenId, 0, 0,  block.timestamp));

        IBasePositionManager.MintParams memory mintParams = _setPartialMintParamsFromPosition(_nftPositionManager, _tokenId);
    ...
    */
    function _collect (uint256 _lockId, address _recipient, uint128 _amount0Max, uint128 _amount1Max) private returns(uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) {
        ...
        if (userLock.ucf == 0) { // No Protocol fee
            (amount0, amount1) = userLock.nftPositionManager.collect(IBasePositionManager.CollectParams(userLock.nft_id, _recipient, _amount0Max, _amount1Max));
        } else { // Protocol fees
            (,,address _token0,address _token1,,,,,,,,) = userLock.nftPositionManager.positions(userLock.nft_id);
            userLock.nftPositionManager.collect(IBasePositionManager.CollectParams(userLock.nft_id, address(this), _amount0Max, _amount1Max));
    ...
    */
    function increaseLiquidity(uint256 _lockId, IBasePositionManager.IncreaseLiquidityParams calldata params) external payable override nonReentrant returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
        Lock memory userLock = LOCKS[_lockId];
    ...
    // */
    // function decreaseLiquidity(uint256 _lockId, IBasePositionManager.DecreaseLiquidityParams calldata params) external payable override nonReentrant returns (uint256 amount0, uint256 amount1) {
    //     isLockAdmin(_lockId);
    //     Lock memory userLock = LOCKS[_lockId];
    //     require(userLock.nft_id == params.tokenId, 'NFT ID');
    ...
    //     userLock.nftPositionManager.collect(IBasePositionManager.CollectParams(userLock.nft_id, msg.sender, type(uint128).max, type(uint128).max));
    ...
    */
    ...
    function _setPartialMintParamsFromPosition (IBasePositionManager _nftPositionManager, uint256 _tokenId) private view returns (IBasePositionManager.MintParams memory) {
        IBasePositionManager.MintParams memory m;
          IBasePositionManager.MintParams memory m;
    ...
    */
    function _getLiquidity (IBasePositionManager _nftPositionManager, uint256 _tokenId) private view returns (uint128) {
